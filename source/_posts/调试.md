...
### Charles 可用本地文件替换线上文件（不推荐）

找到 `Map Local` 选项，具体看 Charles 官网

本质是替换 js css 乃至 html 文件实现线上调试的效果，但现在线上文件都是 webpack 之类编译过的，不推荐该方法

### 用 SwitchyOmega 将线上代理到本地（推荐）

打开线上地址，开启本地服务比如 `http://localhost:8000`，代理线上地址到本地，方便本地修改代码，缺点就是每次修改都要手动刷新
